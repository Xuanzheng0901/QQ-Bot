...
async def handle_func(bot: Bot, event: Event, msg: GroupMessageEvent, args: Message = CommandArg()) -> None:
    ...
    async def album_send(album: jm_entity.JmAlbumDetail):
        await bot.send(event=msg, message=Message(f"正在发送..."))
        ...
        for index, path in enumerate(img_paths):
            ...
            if sent_count % 30 == 0:  # 每30张图发送一次,一次上传太多图片会超时
                ...
                node_list.insert(0, MessageSegment.node_custom(
                    user_id=usr_id,
                    nickname=usr_name,
                    content=Message(
    ...
    def jm_download_task(album_id: int):
        try:
    ...
    async def download_and_send():
        jm_thread = threading.Thread(target=jm_download_task, args=(num, ), daemon=False)
        ...
        # 这里result可能从队列中获取的: 
        # jm_entity.JmAlbumDetail: 下载成功返回的本子信息 
        # str: 下载失败返回的错误信息字符串
    ...
    if num:  # 获取消息中除了/jm之外的文本
        ...
        try:
            ...
            # 如果获取不到直接结束处理
            await JM.finish(f"发生错误:{str(e)}")
            
        txt = MessageSegment.text(
                f'jm{album.album_id}\n'
        ...
        await bot.send(event=msg, message=Message(txt))
